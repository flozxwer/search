...
def concat_ost_steps(solution_trace: Dict[int, Dict[str, str]]) -> Tuple[str, int]:
    ...
    """返回：连接的一步思维步骤和下一个一步思维步骤的ID"""

    # 获取 solution_trace 中的最后一条记录
    last_tuple = list(solution_trace.items())[-1]
    last_tuple_id, last_tuple_recording = last_tuple[0], last_tuple[1]
    assert "ost_step" in last_tuple_recording.keys() # 确保最后一条记录中包含 'ost_step' 键
    # 如果 'ost_step' 中有记录的步骤
    if len(last_tuple_recording["ost_step"]) > 0:
        solution_trace_str = "" # 初始化一个空字符串用于拼接步骤文本
        # 遍历 'ost_step' 中的所有步骤，拼接每个步骤的文本
...
def make_hint(
    ...
    # 如果节点类型是 SUBQUESTION（子问题）或 RE_SUBANSWER（重新子答案）
    if node_type in [Node_Type.SUBQUESTION, Node_Type.RE_SUBANSWER]:
        hint = ""

        # 遍历 solution_trace 中的每个步骤
        for subquestion_id, solution_step in solution_trace.items():
            if subquestion_id == 0:
                continue

            assert subquestion_id > 0  # 确保步骤ID大于0
            assert (
                "subquestion" in solution_step.keys()
                and "subanswer" in solution_step.keys()
            )  # 确保每个步骤中包含子问题和子答案
...
class RstarLanguageNode(MCTS_Node):
    def __init__(
        ...
        node_type: Node_Type,
        verbose: bool = False,
        # --- For instantiating root node ---  根节点
        node_value: float = None,       # 节点的值
        disable_a5: bool = None,        # 禁用 A5 规则
        user_question: str = None,      # 用户提问的内容
        max_depth_allowed: int = None,  # 最大允许的深度
        disable_a1: bool = None,        # 禁用 A1 规则
        # --- For instantiating REPHRASED_USER_QUESTION node ---  A5
        rephrased_user_question: str = None,
        # ------------------------------------------------------
        expected_answer: str = None,
        # --- For instantiating DIRECT_ANSWER node ---  A2
        direct_answer: str = None,
        # --------------------------------------------
        # --- For instantiating SUBQUESTION node ---  A3
        subquestion: str = None,
        subanswer: str = None,
        is_new_subquestion: bool = None,
        # ------------------------------------------
        # --- For instantiating RE_SUBANSWER node ---  A4
        re_subanswer: str = None,
        # -------------------------------------------
        # --- For instantiating OST_STEP node ---  A1
        ost_step: str = None,
