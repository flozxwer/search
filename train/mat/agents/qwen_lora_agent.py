...
class QwenLoRAgent:
    ...
    def infer_for_rollout(self, obs):
        actions, action_tokens = self.get_actions(obs)  #当前状态下的动作和其编码
        
        if self.algo == "APPO": # 策略梯度 Actor-Critic
            values = self.get_action_values(obs) # 值函数 Critic网络
            values = values.float().cpu().numpy()
            action_log_probs, _ = self.get_joint_action_log_probs(obs, action_tokens, batch_infer=True) # 策略（动作的对数概率） Actor网络 （, _用来接受不需要的值并丢弃）
            action_tokens = action_tokens.int().cpu().numpy()
            action_log_probs = action_log_probs.float().cpu().numpy()
            log_probs = action_log_probs
        elif self.algo == "TPPO": # 截断策略 PPO
            values = self.get_token_values(obs, action_tokens).squeeze(-1) # 状态-动作对的值函数 Critic 网络(Q值函数)
            pi_logits, _ = self.get_token_logits(obs, action_tokens, batch_infer=True) # 未归一化的动作 logits
            pi_log_softmax = torch.log_softmax(pi_logits, dim=-1) # 归一化
            token_log_probs = torch.gather(pi_log_softmax, -1, action_tokens.unsqueeze(-1)).squeeze(-1) # 选出与 action_tokens 对应的对数概率值

            values = values.float().cpu().numpy()
            action_tokens = action_tokens.int().cpu().numpy()
            token_log_probs = token_log_probs.float().cpu().numpy()
            log_probs = token_log_probs
        elif self.algo == "GRPO": # 不使用值函数训练，直接优化Actor网络的策略 KL散度
            values = np.zeros((obs.shape[0],)) # fake values, grpo does not use critic 值函数被忽略 values处理化为0
            action_log_probs, _ = self.get_joint_action_log_probs(obs, action_tokens, batch_infer=True) # 和APPO一样
            action_tokens = action_tokens.int().cpu().numpy()
            action_log_probs = action_log_probs.float().cpu().numpy()
            log_probs = action_log_probs
        else:
