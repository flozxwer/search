...
class LanguageBuffer(object):
    ...
    # 经验回放（将每个时间步的 观测、动作、奖励 等数据存储到 缓冲区（buffer） 中）
    # APPO和GRPO都是这个
    def insert_appo(self, obs, actions, value_preds, rewards, masks, action_tokens, action_log_probs):
        ...
        self.masks[self.step + 1] = masks.copy() # 掩码数据（是否结束的标志）
        self.action_tokens[self.step] = action_tokens.copy()
        self.action_level_v_values[self.step] = value_preds.copy() # 当前时间步的价值
        self.action_level_log_probs[self.step] = action_log_probs.copy() # 动作的对数概率

        self.step = (self.step + 1) % self.episode_length     # 更新步数
    ...
    def insert_tppo(self, obs, actions, value_preds, rewards, masks, action_tokens, token_log_probs):
        ...
        self.action_tokens[self.step] = action_tokens.copy()
        self.tppo_values[self.step] = value_preds.copy() # 当前时间步的价值
        self.tppo_log_probs[self.step] = token_log_probs.copy() # 动作的对数概率
    ...
    def batch_process_appo(self, next_value):
        self.action_level_v_values[-1] = next_value # next_value 是下一状态的值估计
        gae = 0
        # 反向传播
        for step in reversed(range(self.episode_length)): # 从最后一个时间步（episode_length - 1）开始，倒着遍历至第一个时间步（0）
            delta = self.rewards[step] \
                + self.gamma * self.action_level_v_values[step + 1] * self.masks[step + 1] \
                    - self.action_level_v_values[step] # 时间差分（TD）误差
            gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
            self.action_level_returns[step] = self.action_level_v_values[step] + gae # 回报
            self.action_level_advantages[step] = gae # 优势
            
    # 标准化奖励（均值为0，标准差为1）
    def batch_process_grpo(self):
        self.action_level_advantages = (self.rewards - np.mean(self.rewards)) / (np.std(self.rewards) + 1e-8)
        # 结构：（奖励 - 奖励的均值）（去中心化） / （奖励的标准差 + 小的常数（避免分母为0））
